...
function redConverter() {
    ...
    for (let i = 0; i < handler.getShape()[0]; i++) {
        for (let j = 0; j < handler.getShape()[1]; j++) {
...
function merge(alphaFirst, alphaSecond) {
    ...
    for (let i = 0; i < catHandler.getShape()[0]; i++) {
        ...
        for (let j = 0; j < catHandler.getShape()[1]; j++) {
            let pixel = [0, 0, 0];
            pixel[0] = Math.round(catPixels[i][j][0]* alphaFirst + dogPixels[i][j][0] * alphaSecond);
            pixel[1] = Math.round(catPixels[i][j][1] * alphaFirst + dogPixels[i][j][1] * alphaSecond);
            pixel[2] = Math.round(catPixels[i][j][2] * alphaFirst + dogPixels[i][j][2] * alphaSecond);
            row.push(pixel);
        }
        pixels.push(row);
    }
